    ...
    public function getTabs(): array
    {
        return [
            'belum_selesai' => Tab::make('Hasil Belum Selesai')
                ->modifyQueryUsing(fn (Builder $query) => $query->whereHas('hasilLingkungans', fn ($q) => $q->whereNull('hasil_parameter')))
                ->icon('heroicon-m-clock')
                ->badge(fn () => PendaftarLingkungan::whereHas('hasilLingkungans', fn ($q) => $q->whereNull('hasil_parameter'))->count())
                ->badgeColor('warning'),
            'sudah_selesai' => Tab::make('Hasil Sudah Selesai')
                ...
                ->badge(fn () => PendaftarLingkungan::whereDoesntHave('hasilLingkungans', fn ($q) => $q->whereNull('hasil_parameter'))->count())
                ->badgeColor('success'),
